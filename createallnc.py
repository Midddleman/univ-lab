...
# --- 读入数据 ---
basin = xr.open_dataset("basin master.nc")
region = xr.open_dataset("region master.nc")
...
def generate_classes(ds):
    ...
    out["grain_total"] = ds["rice"] + ds["wheat"] + ds["maize"]

    out["econ_total"] =  ds["sugarcrops"] + ds["oilcrops"] + ds["othercrops"]
    
    out["grassland_total"] = ds["pastr"] + ds["range"]
